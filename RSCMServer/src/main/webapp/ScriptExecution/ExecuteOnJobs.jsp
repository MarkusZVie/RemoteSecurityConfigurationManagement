			...
			<div class="w3-col l6 w3-padding-large">
				...
				<table class='w3-table w3-striped w3-hoverable' style="width:775px; overflow: scroll;">
					<thead>
						...
						<td>
						<input type="text" id="searchJob" style="width: 100%" name="searchJob" placeholder="Search by Name" width="100%"/>
							<button onclick="loadJobList(); return false;">Search</button></td>
	...
	<footer class="w3-center w3-content content w3-padding-16">
		...
		<script>
			...
			function getFileList() {
						...
						for (var i = 0; i < responsArray.length; i++) {   
							...
							if(jobIdParameter>=0){
								...
								if(responsArray[i]['executionPercentageNumbers']!=""){
									...
									onclickString += "?id="+ jobIdParameter;
									onclickString += "&table=job";
				...
				data.append("table", "job");
				data.append("identifier", jobIdParameter);
			...
			function removeAssignment(){
				...
				if(window.location.href.indexOf('?jobId=') != -1){
					...
					jobIdParameter=-1;
					document.getElementById("updateAssignmentButtonDiv1").innerHTML = "<input type='submit' value='Assign selected Jobs to selected Scripts (checkboxes)' style=\"width: 100%;\" class=\"w3-bar-item marginRight w3-button w3-teal\" >";
					document.getElementById("updateAssignmentButtonDiv2").innerHTML = "<input type='submit' value='Assign selected Jobs to selected Scripts (checkboxes)' style=\"width: 100%;\" class=\"w3-bar-item marginRight w3-button w3-teal\" >";
			...
			function loadJobList() {
				...
				var xhr = new XMLHttpRequest();
				xhr.open("POST", "/ScriptExecution/getJobList");
				xhr.onload = function() {
					var responsArray = JSON.parse(xhr.responseText); //parse Json
					var contentString = ""; //build table content
					var onclickURL = location.protocol + '//' + location.host + location.pathname; //GetURL
					for ( var i in responsArray) {
						if(jobIdParameter == responsArray[i]['jobId']){
					...
					document.getElementById("jobListContent").innerHTML = contentString; //set table content
					getFileList();
				}
			...
			function updateAssignment() {
				var xhr = new XMLHttpRequest();
				xhr.open("POST", "/ScriptExecution/updateJobAssignment");
				xhr.onload = function() {
					document.getElementById("response").innerHTML = xhr.responseText;
					getFileList();
					loadJobList();
