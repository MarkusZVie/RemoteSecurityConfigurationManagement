	...
	<div class="w3-content content " style="padding-top: 70px;">
		...
		<div class="w3-row" id="menu">
			<div class="w3-col l6 w3-padding-large">
				<h2 class="w3-center">Environmentthreats</h2>
				<table class='w3-table w3-striped w3-hoverable' style="width:775px; overflow: scroll;">
					<thead>
						...
						<td>
						<input type="text" id="searchEnvironmentthreat" style="width: 100%" name="searchEnvironmentthreat" placeholder="Search by Title" width="100%"/>
							<button onclick="loadEnvironmentthreatList(); return false;">Search</button></td>
							
						<td style="font-size: 12px;">Threat Description</td>
						<td style="font-size: 12px;">Expected Problem</td>
						<td style="font-size: 12px;">ThreatLevel</td>
						<td style="font-size: 12px;">Environment Number</td>
	...
	<footer class="w3-center w3-content content w3-padding-16">
		...
		<script>
			...
			function getFileList() {
				...
				var environmentthreatIdParameter ="";
				var url = new URL(window.location.href);
				if(window.location.href.indexOf('?environmentthreatId=') != -1){
					environmentthreatIdParameter=url.searchParams.get("environmentthreatId");
					document.getElementById("headLineContent").innerHTML = baseHeadLineContent + "<th>Assigned</th><th>Progress</th>";
				}else if(window.location.href.indexOf('&environmentthreatId=') != -1){
					environmentthreatIdParameter=url.searchParams.get("environmentthreatId");
					document.getElementById("headLineContent").innerHTML = baseHeadLineContent + "<th>Assigned</th><th>Progress</th>";
				}else{
					environmentthreatIdParameter=-1;
				...
				xhr.onload = function() {
					...
					if (xhr.status == 200) {
						...
						for (var i = 0; i < responsArray.length; i++) {   
							...
							//File settings
							if(environmentthreatIdParameter>=0){
								...
								if(responsArray[i]['executionPercentageNumbers']!=""){
									...
									onclickString += "?id="+ environmentthreatIdParameter;
									onclickString += "&table=environmentthreat";
				...
				data.append("table", "environmentthreat");
				data.append("identifier", environmentthreatIdParameter);
			...
			function removeAssignment(){
				...
				if(window.location.href.indexOf('?environmentthreatId=') != -1){
					...
					environmentthreatIdParameter=-1;
					document.getElementById("updateAssignmentButtonDiv1").innerHTML = "<input type='submit' value='Assign selected Environmentthreats to selected Scripts (checkboxes)' style=\"width: 100%;\" class=\"w3-bar-item marginRight w3-button w3-teal\" >";
					document.getElementById("updateAssignmentButtonDiv2").innerHTML = "<input type='submit' value='Assign selected Environmentthreats to selected Scripts (checkboxes)' style=\"width: 100%;\" class=\"w3-bar-item marginRight w3-button w3-teal\" >";
			    ...
			    console.log(environmentthreatIdParameter);  
			    console.log("environmentthreat");  
		        var formData = new FormData();
				formData.append("uncheckedFileNameAssignments", uncheckedAssignments);
				formData.append("entityID", environmentthreatIdParameter);
				formData.append("table", "environmentthreat");
			...
			function loadEnvironmentthreatList() {
				...
				var xhr = new XMLHttpRequest();
				xhr.open("POST", "/ScriptExecution/getEnvironmentthreatList");
				xhr.onload = function() {
					var responsArray = JSON.parse(xhr.responseText); //parse Json
					var contentString = ""; //build table content
					var onclickURL = location.protocol + '//' + location.host + location.pathname; //GetURL
					for ( var i in responsArray) {
						if(environmentthreatIdParameter == responsArray[i]['environmentthreatId']){
							contentString += "<tr style='background-color: #bde9ba;'>";
						}else{
							contentString += "<tr>";
						}						
						
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">" + responsArray[i]['environmentthreatId']+ "</td>";
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">" + responsArray[i]['environmentthreatTitle']	+ "</td>";
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">"	+ responsArray[i]['environmentthreatDescription'] + "</td>";
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">"	+ responsArray[i]['environmentthreatExpectedProblem'] + "</td>";
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">"	+ responsArray[i]['environmentthreatThreatLevel'] + "</td>";
						contentString += "<td onclick=\"window.location='" + onclickURL + "?"+"environmentthreatId="+responsArray[i]['environmentthreatId']+"'\">"	+ responsArray[i]['environmentthreatEnvironments'] + "</td>";
						if(environmentthreatIdParameter<0){
							contentString += "<td><input type='checkbox' name='entityAssignment' value='"+responsArray[i]['environmentthreatId']+"'></td>";
						}
						
						contentString += "</tr>";
					}
					document.getElementById("environmentthreatListContent").innerHTML = contentString; //set table content
					getFileList();
				}
			...
			function updateAssignment() {
				var xhr = new XMLHttpRequest();
				xhr.open("POST", "/ScriptExecution/updateEnvironmentthreatAssignment");
				xhr.onload = function() {
					document.getElementById("response").innerHTML = xhr.responseText;
					getFileList();
