...
$password =  ConvertTo-SecureString 'vFhJtMtwVGpqz5kg' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("vFhJtMtwVGpqz5kg{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'kCHi0I1tj=xCZ8Ca'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'mTyPMRjBqKDOD=f8'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("mTyPMRjBqKDOD=f8{ENTER}")
...
$params = @{"applikationKey"="VkbpPG7Bd7V99RpRT=uS4qxWiwKP13BJ";
