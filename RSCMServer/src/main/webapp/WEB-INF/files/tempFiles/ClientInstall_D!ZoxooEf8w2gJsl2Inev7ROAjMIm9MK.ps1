...
$password =  ConvertTo-SecureString 'uGmVv=rLd9jEh9M0' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("uGmVv=rLd9jEh9M0{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'rr!c6x9BCvriuglb'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'gsvNNRVP6Di8JJbX'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("gsvNNRVP6Di8JJbX{ENTER}")
...
$params = @{"applikationKey"="D!ZoxooEf8w2gJsl2Inev7ROAjMIm9MK";
