...
$password =  ConvertTo-SecureString 'NYVHi58eVcADI1IA' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("NYVHi58eVcADI1IA{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N '5l31zGBwz3QNTVj9'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'u8BcedxVtaRWqsnC'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("u8BcedxVtaRWqsnC{ENTER}")
...
$params = @{"applikationKey"="KE2jJisJPVAws46eUQPeLoh9eLmQSwrq";
