...
$password =  ConvertTo-SecureString 'B2PRlrr6YVI9Ess0' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("B2PRlrr6YVI9Ess0{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'Z7SforXwBdRbwGHb'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'wIlaF3Ijw7gKl7XD'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("wIlaF3Ijw7gKl7XD{ENTER}")
...
$params = @{"applikationKey"="cD8TmyQ1MGlOvrpQlDhFFK4MMYVwB559";
