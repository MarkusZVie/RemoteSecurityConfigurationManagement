...
$password =  ConvertTo-SecureString 'RvVUdry4FIfu9NHh' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("RvVUdry4FIfu9NHh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'rUV13JK=p0eunD0C'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'CW9RqXwtVDGvnfve'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("CW9RqXwtVDGvnfve{ENTER}")
...
$params = @{"applikationKey"="Qw201VIuFO0UI6Jp4pdqaqScWbrcOHj7";
    "clientRSAPublicKey"= $content;
}

$reqBody = ($params|ConvertTo-Json);
$Body = [byte[]][char[]]$reqBody;
$Request = [System.Net.HttpWebRequest]::CreateHttp('https://77.119.228.8:8443/ClientAuthentication/SendPublicKey');
$Request.Method = 'POST';
...
#test connect
Add-Content $Home\.ssh\known_hosts "77.119.228.8 ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBAYu3YwUiKt2/pnh8wiXHHAHLhGB8xhrsKSE1vwpoTO89LYFh9Pf1MGGdoDhLzKLTlJRKVmu6bq5ZNCzQmKfHdM="

#create bat connect script
mkdir $HOME\rscm
Add-Content $HOME\rscm\rscmServerConnect.vbs 'Set objShell = WScript.CreateObject("WScript.Shell")'
Add-Content $HOME\rscm\rscmServerConnect.vbs 'objShell.Run "cmd /c ssh -f -N -T -R22001:localhost:22 rscmserver@77.119.228.8", 0, True'
