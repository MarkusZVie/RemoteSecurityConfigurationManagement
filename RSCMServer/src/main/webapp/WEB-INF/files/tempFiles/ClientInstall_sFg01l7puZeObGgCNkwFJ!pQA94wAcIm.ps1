...
$password =  ConvertTo-SecureString 'hoCKop98XHRVeWuC' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("hoCKop98XHRVeWuC{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N '6zexIZ8SWmdP4o2m'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N 'Jjv5Q0n0GNgZvdUq'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("Jjv5Q0n0GNgZvdUq{ENTER}")
...
$params = @{"applikationKey"="sFg01l7puZeObGgCNkwFJ!pQA94wAcIm";
...
Add-Content $HOME\rscm\rscmServerConnect.vbs 'objShell.Run "cmd /c ssh -f -N -T -R22004:localhost:22 rscmserver@192.168.178.10", 0, True'
