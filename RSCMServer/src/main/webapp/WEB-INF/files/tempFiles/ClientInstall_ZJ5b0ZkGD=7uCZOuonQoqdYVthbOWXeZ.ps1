...
$password =  ConvertTo-SecureString 'iV92!jZDbXbzJ=FM' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("iV92!jZDbXbzJ=FM{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'LlTkPYR7nEOHsxCh'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N '5vx517JAV5=kbarp'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("5vx517JAV5=kbarp{ENTER}")
...
$params = @{"applikationKey"="ZJ5b0ZkGD=7uCZOuonQoqdYVthbOWXeZ";
