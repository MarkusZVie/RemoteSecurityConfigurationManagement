...
$password =  ConvertTo-SecureString 'MHHXkItD9G9f1STg' -asplaintext -force
...
Start-Sleep -m 1000
$wshell.SendKeys("MHHXkItD9G9f1STg{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("mkdir C:\Users\rscm\.ssh{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("ssh-keygen.exe -f C:\Users\rscm\.ssh\id_rsa -N 'OZ!PbatqI9Ln1Q3A'{ENTER}")
Start-Sleep -m 1000
$wshell.SendKeys("exit{ENTER}")
#create own keys
ssh-keygen.exe -f $HOME\.ssh\id_rsa -N '!FlPsuZpor5SPUPU'

$wshell2 = New-Object -ComObject Wscript.Shell
$wshell2.Run("ssh-add.exe $HOME\.ssh\id_rsa")
Start-Sleep -m 1000
$wshell2.SendKeys("!FlPsuZpor5SPUPU{ENTER}")
...
$params = @{"applikationKey"="sVBSh6PORu4ajWaVH!1mRhrJ5Bko5pzz";
